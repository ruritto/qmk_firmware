...
- **タスク追加**: 右下の「＋」ボタン → タスク名 / チーム / デバイス種別 / 担当者 / 開始日 / 終了日 を入力(どのタブからでも同じ)
- **全体予定**: ガントの日付ヘッダー直下の欄をタップして追加。1日1件・全角6文字(半角12文字)まで・赤字の縦書きで表示
- **一覧タブ**: 班ごとにタスクを開始日順で上から表示するログビュー。行タップで編集
...
events:   id / date(1日1件) / title(全角6文字まで) — ガント上部に赤字で表示する全体予定
```
